import { getSupabaseServerClient } from './supabase-server';

/**
 * Server-side Credential Resolver matching exact Supabase SQL schema.
 * Categories allowed by DB check constraint: 'llm_reasoning', 'email', 'whatsapp', 'voice_call', 'payment_gateway'
 * Exact provider_name filtering supported for case-sensitive queries:
 *   LLM: provider_name = 'Sarvam AI Agent', category = 'llm_reasoning'
 *   Voice: provider_name = 'Voice agent', category = 'voice_call'
 */
export async function executeWithCredential(category, executionFn, leakId = null, providerName = null) {
  const supabase = getSupabaseServerClient();

  // 1. Primary lookup: exact category + status='active' (with exact provider_name if provided)
  let credentials = null;
  if (providerName) {
    const { data } = await supabase
      .from('api_credentials')
      .select('*')
      .eq('category', category)
      .eq('provider_name', providerName)
      .eq('status', 'active')
      .order('priority', { ascending: true });
    if (data && data.length > 0) {
      credentials = data;
    }
  }

  // 2. Fallback: query active keys in category regardless of provider name casing/variation
  if (!credentials || credentials.length === 0) {
    const { data } = await supabase
      .from('api_credentials')
      .select('*')
      .eq('category', category)
      .eq('status', 'active')
      .order('priority', { ascending: true });
    if (data && data.length > 0) {
      credentials = data;
    }
  }

  // 3. Fallback: if keys were temporarily marked 'failed' by a previous runtime error, auto-recover non-disabled keys
  if (!credentials || credentials.length === 0) {
    const { data: recovered } = await supabase
      .from('api_credentials')
      .select('*')
      .eq('category', category)
      .neq('status', 'disabled')
      .order('priority', { ascending: true });

    if (recovered && recovered.length > 0) {
      await supabase
        .from('api_credentials')
        .update({ status: 'active', last_error: null })
        .eq('category', category)
        .neq('status', 'disabled');
      credentials = recovered;
    }
  }

  if (!credentials || credentials.length === 0) {
    console.error(`No active credentials found for category [${category}]${providerName ? ` provider [${providerName}]` : ''}`);
    await handleEscalation(supabase, leakId, `No active API credentials configured for category: ${category}${providerName ? ` (${providerName})` : ''}`);
    return { success: false, escalated: true, error: `No active credentials for category ${category}` };
  }

  for (const cred of credentials) {
    try {
      const apiKey = cred.encrypted_key;
      if (!apiKey || typeof apiKey !== 'string' || apiKey.trim() === '') {
        throw new Error(`Credential ID ${cred.id} (${cred.provider_name}) has a NULL or EMPTY encrypted_key in the api_credentials table.`);
      }
      const result = await executionFn(apiKey.trim(), cred);

      await supabase
        .from('api_credentials')
        .update({ last_used_at: new Date().toISOString(), last_error: null })
        .eq('id', cred.id);

      return { success: true, data: result, provider: cred.provider_name };
    } catch (err) {
      const errorMessage = err.message || 'API execution failed';
      const isRateLimit = errorMessage.toLowerCase().includes('rate') || errorMessage.includes('429');
      const newStatus = isRateLimit ? 'rate_limited' : 'failed';

      console.warn(`Credential ID ${cred.id} failed for category [${category}]: ${errorMessage}. Retrying next priority key...`);

      await supabase
        .from('api_credentials')
        .update({
          status: newStatus,
          last_error: errorMessage,
        })
        .eq('id', cred.id);
    }
  }

  console.error(`All credentials in category [${category}] exhausted. Escalating leak ${leakId}`);
  await handleEscalation(supabase, leakId, `All API keys in category ${category} failed or rate-limited.`);
  return { success: false, escalated: true, error: `All credentials in category ${category} exhausted.` };
}

export async function handleEscalation(supabase, leakId, reason) {
  if (!leakId) return;

  await supabase
    .from('leaks')
    .update({
      status: 'needs_manual_diagnosis',
      root_cause: 'unknown',
    })
    .eq('id', leakId);

  await supabase
    .from('audit_log')
    .insert([
      {
        leak_id: leakId,
        event_timestamp: new Date().toISOString(),
        event_type: 'escalated',
        detail: `System escalated leak to operator queue. Reason: ${reason}`,
        outcome: 'Needs Manual Diagnosis',
      },
    ]);
}

export async function getActiveCredential(category, providerName = null) {
  const supabase = getSupabaseServerClient();
  let query = supabase
    .from('api_credentials')
    .select('*')
    .eq('category', category)
    .eq('status', 'active');

  if (providerName) {
    query = query.eq('provider_name', providerName);
  }

  const { data: credentials, error } = await query
    .order('priority', { ascending: true })
    .limit(1);

  if (error || !credentials || credentials.length === 0) {
    return null;
  }
  return credentials[0];
}
